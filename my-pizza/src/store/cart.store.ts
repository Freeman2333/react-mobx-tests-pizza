...
import { ICartItem, ICartStore } from '../types/cart.types';
import { getTotalCount, getTotalPrice } from '../utils/counters';
import { isEqualObjects } from '../utils/equals';

const carryTotalCountPrice =
  <T extends Function, O extends Function>(fn1: T, fn2: O) =>
  (items: ICartItem[]) => {
...
class CartStore implements ICartStore {
  ...
  addPizzaToCart(newObj: ICartItem) {
    ...
    const findItemIndexCart = cartItem.findIndex((item: any) =>
